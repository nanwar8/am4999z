...
for j in j_vals:
    ...
    t_axis = np.arange(len(averages_p1)) * timestep

    data_p1 = np.array(averages_p1[start_idx:])
    data_p2 = np.array(averages_p2[start_idx:])
    data_m  = np.array(averages_m[start_idx:])
    
# p1
    plt.figure()
    plt.plot(t_axis[start_idx:], averages_p1[start_idx:])
    plt.title(f"time series of $\\bar{{p1}}$ (Δt = {timestep}, j = {j})")
    plt.xlabel("time")
    plt.ylabel("number of p1 molecules")
    
    plt.figure()
    plt.hist(data_p1, bins=make_bins(data_p1, bin_width=1, start=0))
    plt.title(f"histogram of $\\bar{{p1}}$ (Δt = {timestep}, j = {j})")
...
# p2
    ...
    plt.figure()
    plt.hist(data_p2, bins=make_bins(data_p2, bin_width=1, start=0))
    plt.title(f"histogram of $\\bar{{p2}}$ (Δt = {timestep}, j = {j})")
...
# m
    plt.figure()
    plt.plot(t_axis[start_idx:], averages_m[start_idx:])
    plt.title(f"time series of $\\bar{{m}}$ (Δt = {timestep}, j = {j})")
    plt.xlabel("time")
    plt.ylabel("number of m molecules")
    
    plt.figure()
    plt.hist(data_m, bins=make_bins(data_m, bin_width=1, start=0))
    plt.title(f"histogram of $\\bar{{m}}$ (Δt = {timestep}, j = {j})")
